...
trap_init(void)
{
	...
	SETGATE(idt[T_BRKPT], 0, GD_KT, trap_bpoint, 3);
	SETGATE(idt[T_OFLOW], 0, GD_KT, trap_oflow, 0);
	SETGATE(idt[T_BOUND], 0, GD_KT, trap_brexc, 0);
	SETGATE(idt[T_ILLOP], 0, GD_KT, trap_invop, 0);
	SETGATE(idt[T_DEVICE], 0, GD_KT, trap_dna, 0);
	SETGATE(idt[T_DBLFLT], 0, GD_KT, trap_dfault, 0);
	SETGATE(idt[T_TSS], 0, GD_KT, trap_invtss, 0);
	SETGATE(idt[T_SEGNP], 0, GD_KT, trap_segnp, 0);
	SETGATE(idt[T_STACK], 0, GD_KT, trap_sfault, 0);
	SETGATE(idt[T_GPFLT], 0, GD_KT, trap_gpfault, 0);
	SETGATE(idt[T_PGFLT], 0, GD_KT, trap_pfault, 0);
	SETGATE(idt[T_FPERR], 0, GD_KT, trap_fpuerr, 0);
	SETGATE(idt[T_ALIGN], 0, GD_KT, trap_alignchk, 0);
	SETGATE(idt[T_MCHK], 0, GD_KT, trap_mchk, 0);
	SETGATE(idt[T_SIMDERR], 0, GD_KT, trap_simdexc, 0);	
...
trap_init_percpu(void)
{
	...
	SETTSS((struct SystemSegdesc64 *)(&gdt[(GD_TSS0 >> 3) + 2*cpunum()]),
...
page_fault_handler(struct Trapframe *tf)
{
    ...
    if(curenv->env_pgfault_upcall) {
        uintptr_t xrsp = UXSTACKTOP;

        if(tf->tf_rsp < UXSTACKTOP && tf->tf_rsp >= UXSTACKTOP - PGSIZE) {
            xrsp = tf->tf_rsp - 64;
        }

        xrsp -= sizeof(struct UTrapframe);

        struct UTrapframe *u = (struct UTrapframe*) xrsp;
