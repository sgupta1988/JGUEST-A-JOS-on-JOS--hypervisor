...
char *nextfree; // virtual address of next byte of free memory
...
x64_vm_init(void)
{
	...
	transDespList = boot_alloc(TOTAL_TX_DESC*16);	
	rcvDespList = boot_alloc(TOTAL_RX_DESC*16);
	guest_rcvDespList = boot_alloc(GUEST_TOTAL_RX_DESC*16);
    page_init();
    ...
    boot_map_segment(boot_pml4e, (uint64_t)guest_rcvDespList, (64*16), PADDR(guest_rcvDespList), PTE_W | PTE_P | PTE_U);
...
mem_init_mp(void)
{
	...
	for (i=0; i < NCPU; i++)
	{
		uint64_t kstacktop_i = KSTACKTOP - i * (KSTKSIZE + KSTKGAP);
        	boot_map_segment(boot_pml4e, kstacktop_i - KSTKSIZE, KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W | PTE_P);
	        boot_map_segment(boot_pml4e, kstacktop_i- (KSTKSIZE + KSTKGAP), KSTKGAP, PADDR(percpu_kstacks[i]) + KSTKSIZE, PTE_W);
	}
}
...
pages[i].pp_ref = 1; 
//IO hole 
else if (i >= (physaddr_t)(IOPHYSMEM / PGSIZE) && i < (physaddr_t)(EXTPHYSMEM / PGSIZE))
pages[i].pp_ref = 1;
//JOS kernel,page directories and other data strutures allocated by boot_alloc 
else if ((i >= (physaddr_t)(EXTPHYSMEM / PGSIZE)) && (i < (physaddr_t)((EXTPHYSMEM + (uintptr_t)nextfree - KERNBASE) / PGSIZE)))
pages[i].pp_ref = 1; 
else if (i >= BOOT_PAGE_TABLE_START / PGSIZE && i <= BOOT_PAGE_TABLE_END) pages[i].pp_ref = 1; 

else if(i == MPENTRY_PADDR / PGSIZE) pages[i].pp_ref = 1; 
else 
{ pages[i].pp_ref = 0; 
if (last) last->pp_link = &pages[i]; 
else page_free_list = &pages[i]; 
last = &pages[i];
}
...
pml4e_walk(pml4e_t *pml4, const void *va, int create)
{
	...
	pml4e_t *offsetd_ptr_in_pml4t = pml4 + index_in_pml4t;
...
pte_t *
pdpe_walk(pdpe_t *pdpt_base,const void *va,int create)
{        
        uintptr_t index_in_pdpt = PDPE(va);
        pdpe_t *offsetd_ptr_in_pdpt = pdpt_base + index_in_pdpt;
...
// and pdpe_walk.
    ...
    pte_t *
pgdir_walk(pde_t *pgdir_base, const void *va, int create)
{
	uintptr_t index_in_pgdir = PDX(va);
	pde_t *offsetd_ptr_in_pgdir = pgdir_base + index_in_pgdir;
...
check_page_free_list(bool only_low_memory)
{
 ...
 //       cprintf("pp %d",pp);
  //      cprintf("extp %d",EXTPHYSMEM);
   //     cprintf("pgsize %d",PGSIZE);
   //     cprintf("delta` %d",EXTPHYSMEM - PGSIZE);
