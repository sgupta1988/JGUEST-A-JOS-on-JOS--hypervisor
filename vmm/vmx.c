...
static inline bool vmx_check_ept() {
   // cprintf("vmx_check_ept\n");
    ...
    //cprintf("Primary Processor Based COntrol[%x]\n",procbased_ctls_or);
    //cprintf("Primary Processor Based COntrol[%x]\n",procbased_ctls_and);
    //cprintf("Secondary Processor Based Control[%x]\n",procbased_ctls2_or);
    //cprintf("Secondary Processor Based Control[%x]\n",procbased_ctls2_and);
    if(((procbased_ctls_and>>31)&1)&&((procbased_ctls2_and>>2)&1))
    {
	    cprintf("Nested Page Table enabled");    
	    return true;
    }
    else
    {
	    cprintf("Nested Page Table not enabled");    
	    return false;
    }
...
void vmexit() {
    ...
    switch(exit_reason & EXIT_REASON_MASK) {
        ...
        case EXIT_REASON_EPT_VIOLATION:
			...
			//cprintf("\n vmexit: env_type  %d ENV_TYPE_GUEST %d\n",curenv->env_type, ENV_TYPE_GUEST);
             break;
...
//cprintf("\n successfully completed asm stmt in vmrun \n");
    if(tf->tf_es) {
        //cprintf("\n IN if loop \n");
