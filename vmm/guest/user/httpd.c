...
send_data(struct http_request *req, int fd)
{
	// LAB 6: Your code here.
        char buf[512]; //TODO: Handle large files.
        int r;
	struct Fd *fdd;

        if ((r = fd_lookup(fd, &fdd)) < 0)
                return r;
	
	if ((r = devfile.dev_read(fdd, buf, sizeof buf)) < 0)
		return r;
...
send_file(struct http_request *req)
{
	...
	// LAB 6: Your code here.
	if ((fdnum = open(req->url, O_RDONLY)) < 0) {
		if (fdnum == -E_NOT_FOUND)
			send_error(req, 404);
		else
			send_error(req, 400);
		goto end;
...
{
    cprintf("Starting Http Deamon...\n");
    int serversock, clientsock;
    struct sockaddr_in server, client;

    binaryname = "jhttpd";
    cprintf("Creating TCP Socket...\n");
    ...
    cprintf("Filling the socket");
    // Construct the server sockaddr_in structure
    memset(&server, 0, sizeof(server));		// Clear struct
    server.sin_family = AF_INET;			// Internet/IP
    server.sin_addr.s_addr = htonl(INADDR_ANY);	// IP address
    server.sin_port = htons(PORT);			// server port
    cprintf("Binding TCP Socket...\n");
